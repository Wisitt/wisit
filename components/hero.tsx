...
/* helper: attempts smooth scroll to #work or falls back to root hash navigation */
function scrollToWork(): void {
  ...
  if (el) {
    ...
    // fallback: navigate to root with hash (will trigger scroll on mount of that page)
    if (window.location.pathname !== "/") {
      window.location.href = "/#work";
    } else {
      // if currently on root but element not present yet, set hash so mount effect can pick it up
...
export default function Hero() {
  ...
  return (
    <header
      ...
      style={{ ["--px"]: "0px", ["--py"]: "0px" } as React.CSSProperties & Record<string, string>}
      aria-label="Hero"
    >
      <style>{`
        html { scroll-behavior: smooth; } /* prefer global smooth scroll; harmless if already present */
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-8 sm:px-12 lg:px-16">
        <nav className="flex items-center justify-between py-6">
          ...
          <div className={`flex items-center gap-4 ${mounted ? "opacity-100" : "opacity-0"}`}>
            {/* anchor + onClick for robust behavior */}
            <a href="#work" onClick={(e) => { e.preventDefault(); scrollToWork(); }} className="text-sm hover:opacity-80">WORK</a>
            <a href="#contact" onClick={(e) => { e.preventDefault(); const el = document.getElementById("contact"); if (el) el.scrollIntoView({ behavior: "smooth" }); else window.location.hash = "#contact"; }} className="text-sm hover:opacity-80">CONTACT</a>

            <div className="flex items-center gap-2">
              <Link href="/resume" aria-label="Open resume (new page)" className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-md text-sm font-semibold bg-gradient-to-r from-white/95 to-white/75 text-black shadow-[0_6px_18px_rgba(255,255,255,0.06)] hover:scale-105 transform transition focus:outline-none focus:ring-2 focus:ring-white/30">
                <DownloadCloud className="w-4 h-4" />
                <span>Resume</span>
                <span className="ml-1 text-xs bg-black/10 px-2 py-0.5 rounded-full text-black/70 font-medium">PDF</span>
              </Link>

              <Link href="/resume" aria-label="Open resume" className="inline-flex sm:hidden items-center justify-center w-10 h-10 rounded-full bg-white/12 border border-white/10 hover:bg-white/20 transition transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-white/20" title="Resume">
        ...
        <main className="flex flex-col-reverse lg:flex-row items-center gap-12 lg:gap-20 py-10">
          <div className="flex-1 ">
            ...
            <div className={`flex flex-wrap gap-3 items-center ${mounted ? "opacity-100" : "opacity-0"}`}>
              ...
              <a href="#contact" onClick={(e) => { e.preventDefault(); const el = document.getElementById("contact"); if (el) el.scrollIntoView({ behavior: "smooth" }); else window.location.hash = "#contact"; }} className="px-5 py-3 border border-white/20 rounded font-medium bg-white/5">Get in touch</a>
            </div>
